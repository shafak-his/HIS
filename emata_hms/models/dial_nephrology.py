...
class EmHmsDialNephrology(models.Model):
    ...
    analysis_request_ids = fields.Many2many('product.template', 'dial_nephrology_product_analysis_rel','clinic_visit_id', 'product_id', string='Analysis Requests',domain="[('is_medical_analysis', '=', True)]")
    image_request_ids = fields.Many2many('product.template', 'dial_nephrology_product_image_rel', 'clinic_visit_id','product_id', string='Image Requests',domain="[('is_medical_imaging', '=', True)]")
